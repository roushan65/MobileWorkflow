...
export class MovView extends ViewBase {
  state = {};
  handlers = {};
  ...
  constructor(props) {
    ...
    this.state.actionText = this.props.actionText;
    //console.log(this);
  }


  render() {
    //console.log("rendering view ",this.state.id);
    let comId = 0;
    return (
      <React.Fragment key={this.state.id}>
        <View style={this.styles.app}>
          <View style={this.styles.header}>
            ...
            />
            <Text style={this.styles.title}>{this.state.name}</Text>
          </View>
          
          {this.controls.map((comp) => React.cloneElement(comp,{key:comId++}))}
          <Button onPress={() => { this.dispatchEvent("onNext", this) }} title="NextPage" />
        </View>
      </React.Fragment>
    );
  }
